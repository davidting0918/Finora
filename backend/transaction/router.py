...
from backend.core.model.transaction import CreateTransactionRequest, UpdateTransactionRequest, TransactionListQuery, TransactionType, CategoryId, SubCategoryId
...
async def get_transaction_list_route(
    ...
    sort_by: str = Query("transaction_date", description="Sort by field (transaction_date, amount, created_at, updated_at)"),
...
async def delete_transaction_route(transaction_id: str, current_user: Annotated[User, Depends(get_current_active_user)]):
    try:

...
from backend.core.model.transaction import Transaction, TransactionType, transaction_collection
...
class AnalyticsService:
    ...
    async def _calculate_financial_summary(self, transactions: List[Transaction]) -> FinancialSummary:
        ...
        if category_counts:
            ...
            most_frequent_category = {"category_id": most_freq_cat[0], "count": most_freq_cat[1]}
    ...
    async def _calculate_category_breakdown(self, transactions: List[Transaction]) -> List[CategoryAnalytics]:
        ...
        category_data = defaultdict(lambda: {"transactions": [], "subcategories": defaultdict(list)})
    ...
    async def _calculate_spending_trends(
        self, transactions: List[Transaction], query: AnalyticsQuery
    ) -> List[SpendingTrend]:
        """
        ...
        for period, data in sorted(period_data.items()):
            result.append(SpendingTrend(date=period, amount=data["amount"], transaction_count=data["count"]))

...
class TestTransactionCRUD:
    @pytest.mark.asyncio
    async def test_create_single_transaction(
        self, async_client: AsyncClient, session_auth_headers, test_transactions_data
    ):
        """Test creating a single transaction"""
        transaction_data = test_transactions_data["transaction1"]

        response_data, status_code = await create_transaction(async_client, session_auth_headers, transaction_data)
    ...
    async def test_create_multiple_transactions(
        ...
        for tx in tx_list:
            response_data, status_code = await create_transaction(async_client, session_auth_headers, tx)
    ...
    async def test_udpate_transaction(self, async_client: AsyncClient, session_auth_headers, test_transactions_data):
        ...
        response_data, status_code = await create_transaction(async_client, session_auth_headers, old_transaction_data)
        assert status_code == status.HTTP_200_OK
        assert response_data["status"] == 1
        assert response_data["message"] == "Transaction created successfully"
    ...
    async def test_get_single_transaction(
        ...
        response_data, status_code = await get_transaction(created_transaction_id, async_client, session_auth_headers)
    ...
    async def test_get_single_transaction_not_found(
        ...
        response_data, status_code = await get_transaction(non_existent_id, async_client, session_auth_headers)

        assert status_code == status.HTTP_404_NOT_FOUND

    @pytest.mark.asyncio
    async def test_get_transaction_list_default(
        self, async_client: AsyncClient, session_auth_headers, test_transactions_data
    ):
        ...
        response_data, status_code = await get_transaction_list(async_client, session_auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_delete_single_transaction(
        self, async_client: AsyncClient, session_auth_headers, test_transactions_data
    ):
        ...
        assert get_response_data["data"]["is_deleted"] is True
    ...
    async def test_delete_transaction_not_found(
        ...
        response_data, status_code = await delete_transaction(non_existent_id, async_client, session_auth_headers)
    ...
    async def test_create_transaction_invalid_category(
        ...
        response_data, status_code = await create_transaction(async_client, session_auth_headers, invalid_category_data)

        assert status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    @pytest.mark.asyncio
    async def test_create_transaction_invalid_subcategory(
        self, async_client: AsyncClient, session_auth_headers, test_transactions_data
    ):
